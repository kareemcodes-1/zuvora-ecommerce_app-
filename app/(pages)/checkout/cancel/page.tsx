...
export default function CanceledPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white flex flex-col items-center justify-center px-6 overflow-hidden relative">
      ...
      {/* Subtle radial glow — red tint */}
      <div className="pointer-events-none fixed inset-0 flex items-center justify-center">
        <div className="w-[600px] h-[600px] rounded-full bg-red-900 opacity-[0.06] blur-[140px]" />
      </div>
      ...
      <div className="flex flex-col items-center text-center max-w-[480px] gap-[2.5rem]">
        ...
        >
          <div className="w-[72px] h-[72px] rounded-full border border-white/10 flex items-center justify-center bg-white/5">
            <X size={28} strokeWidth={1.25} className="text-white/60" />
          </div>
        </div>
        ...
        >
          ...
          <p className="text-[1rem] text-white/40 font-[300] leading-[1.6]">
            Your order was not completed. Nothing has been charged. You can return to your cart and try again whenever you're ready.
          </p>
        </div>

        {/* CTAs */}
        <div
          className={`flex items-center gap-[1rem] w-full transition-all duration-700 delay-[600ms] ${
            visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          }`}
        >
           <Link
            href="/shop"
            className="next-btn btn-base btn-light !w-full !text-start"
          >
            Browse Products
          </Link>
        </div>
